...
export class HipoContract {
  ...
  public async createWalletFromGateChainAccount(
    privateKeyHex?: string | null
  ): Promise<{ gateWallet: any; gateAddress: string }> {
    const { gateWallet, gateAddress } =
      await gatewallet.createWalletFromGateChainAccount(
        this.signer,
        this?.config[this.chainId]?.GateWalletConfig,
        privateKeyHex || null
      );
  ...
  public signCreateAccountAuthorization() {
    if (!this.gateWallet) {
      throw new Error(
        "Please use hipocontract in advance Createwalletfrometheraccount initialize gatewallet."
      );
    }

    return this.gateWallet.signCreateAccountAuthorization(
      this.provider,
      this.signer
    );
  }
